...
// Server is a read-only viewer over the ManagedNamespaces. It performs no
// mutations; policy is authored with kubectl or GitOps against the same
// Kubernetes objects. The embedded UI (internal/server/ui) drives both views
// from the single list endpoint.
type Server struct {
